...
func New(store storage.IStorage, service service.IServiceManager, log logger.ILogger) *gin.Engine {
	...
	// permissions
	r.POST("/api/v1/permission", h.CreatePermission)
	r.PUT("/api/v1/permission", h.UpdatePermission)
	...
	// user-roles
	r.POST("/api/v1/user-role", h.CreateUserRole)
	r.DELETE("/api/v1/user-role", h.DeleteUserRole)
	r.PUT("/api/v1/user-role", h.UpdateUserRole)
